...
//获取房源标签
function getProduceTagInfo(){
	
	$.ajax({
		type : "POST",
		url : "get_product_all_tag",
		async : false,
		success : function(data){
			
				fillProduceUseTagInfo(data.useList);
				fillProduceUnUseTagInfo(data.unUselist);
...
function fillProduceUseTagInfo(data){
	...
	$.each(data,function(v,k){
		//console.log(k);
		if(k.tagStatus != 0 && k.tagTypeStatus != 2){
			...
			$.each(k.tags, function(v,o){
				...
				if(o.parentTagId == null && o.tagStatus != 0){
					
					s += "<ul><li class='li' >"+o.tagName+"<input class='tag' value='"+o.tagId+"' type='hidden'><ul  class='ul' >";
					
					
					$.each(o.children, function(v,t){
						if(t.parentTagId == o.tagId && t.parentTagId != null && t.tagStatus !=0){
							s+="<li>"+t.tagName+"<input type='hidden' value='"+t.tagId+"' class='zTag'></li>";
						}
					});
					s+="</ul></li></ul>";
				}
					/*s += "<span>"+o.tagName+"<input class='tag' value='"+o.tagId+"' type='hidden'></span>";*/
			});
