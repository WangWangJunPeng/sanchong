(function(w){
	w.Calendar = function (opt) {
		...
		//初始化显示本月信息
		setContent(content, fistWeek, monDaynum, nowDay,dateObj);
	...
	// 初始化日期显示方法
	function setContent(el, fistWeek, monDaynum, nowDay,choose){
		// 留空
		...
		for(var i=1;i <= monDaynum;i++){
			...
			judgeChoose(i,subContent,choose,nowDay);
	...
	function judgeChoose (i,obj,choose,nowDay) {
		var chooseYear = choose.getFullYear();
		var chooseMon = choose.getMonth();
		if (chooseYear > nowyear) {
			obj.classList.add("future");
			obj.className="future";
		} else if (chooseYear <= nowyear && chooseMon > nowmonth) {
			obj.classList.add("future");
			obj.className="future";
		}else if (nowmonth == chooseMon && nowyear == chooseYear) {
			if (i==nowDay) {
